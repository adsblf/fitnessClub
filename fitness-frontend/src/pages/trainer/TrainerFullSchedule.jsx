...
export default function TrainerFullSchedule() {
  const { user } = useAuth();
  // person_id — корректный ID для сравнения с trainer.id из API
  ...
  async function openAttendance(s) {
    ...
    try {
      const r = await visitsApi.sessionsWithVisits({
        from_date: s.date,
        to_date: s.date,
        per_page: 500,
      });
      const detail = r.data.data.find((sd) => sd.id === s.id) ?? null;
      setAttendanceDetail(detail);
    } catch {
      // оставить attendanceDetail = null
    } finally {
  ...
  // Является ли занятие «моим» (тренер — текущий пользователь)
  function isMySession(s) {
    // eslint-disable-next-line eqeqeq
    return s.trainer?.id == myTrainerId;
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-zinc-500">
        <span className="inline-block w-3 h-3 rounded bg-indigo-300 dark:bg-indigo-700 shrink-0" />
        <span>Ваши занятия выделены цветом</span>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4">
        ...
        {filtersActive && (
          <button
            onClick={() =>
              setFilters({ date: "", trainer_id: "", hall_id: "", sort_slots: "", hide_cancelled: true })
            }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-zinc-400 border-b border-zinc-100 dark:border-zinc-800">
                ...
                <th className="text-left px-5 py-2.5 font-normal">Мест / Клиент</th>
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-zinc-50 dark:border-zinc-800 last:border-0 ${
                      mine
                        ? "bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-l-indigo-400 dark:border-l-indigo-600"
                        : i % 2
      ...
      {attendanceSession && (
        ...
        >
          <div
            className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {attendanceLoading ? (
              <div className="p-10 text-center text-sm text-zinc-400">Загрузка данных...</div>
            ) : attendanceDetail ? (
              <div className="p-4">
                <SessionVisitBlock
                  session={attendanceDetail}
                  onUpdated={() => {
                    openAttendance(attendanceSession);
                    fetchSessions();
                  }}
                />
                <button
                  onClick={closeAttendance}
                  className="mt-3 w-full py-2 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800"
                >
                  Закрыть
                </button>
              </div>
              ...
              <div className="p-10 text-center">
                <div className="text-sm text-zinc-400 mb-4">
                  Данные о посещаемости не найдены
                </div>
                <button
                  onClick={closeAttendance}
                  className="py-2 px-4 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-600 dark:text-zinc-400"
                >
                  Закрыть
                </button>
              </div>
...
function CreateSessionModal({ halls, trainers, defaultTrainerId, onClose, onCreated }) {
  const [form, setForm] = useState({
    ...
    ends_at: "",
    hall_id: "",
    trainer_id: defaultTrainerId ? String(defaultTrainerId) : "",
    max_participants: 15,
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {form.type === "personal" && (
            <div>
              ...
              <ClientSearchAutocomplete onSelect={setSelectedClient} placeholder="Поиск по ФИО..." />
              {selectedClient && (
          ...
          <div>
            <label className="block text-xs text-zinc-500 mb-1">Тренер</label>
            <div className="w-full px-3 py-2 text-sm rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800/60 text-zinc-500 dark:text-zinc-400 select-none">
              {trainers.find((t) => t.id === Number(form.trainer_id))?.full_name ?? "—"}
            </div>
          </div>
...
function EditSessionModal({ session, halls, trainers, onClose, onSaved }) {
  ...
  const [selectedClient, setSelectedClient] = useState(
    session.client
      ? { person_id: session.client.id, full_name: session.client.full_name }
      : null
  );
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
